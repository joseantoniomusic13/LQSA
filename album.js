...
function initAlbumSystem(uid) {
  ...
  // Verificar e inicializar misiones diarias del usuario reactivamente
  checkAndGenerateMissions(uid).then(() => {
    const todayKey = new Date().toISOString().split('T')[0];
    db.ref(`users/${uid}/daily_missions/${todayKey}`).on('value', snap => {
      if (snap.exists()) {
        userAlbumData.missions = snap.val();

        // Si la modal de misiones está abierta, redibujarla sin flickering
        const modal = document.getElementById("auth-modal");
        if (modal && modal.innerHTML.includes("MISIONES DIARIAS")) {
          openMissionsUI();
        }
      }
    });
...
const MISSION_POOL = [
  { id: "play_game", desc: "Juega 1 partida hoy (Cualquier modo)", target: 1, reward: 80 },
  { id: "win_machine", desc: "Gana 1 partida contra la máquina", target: 1, reward: 120 },
  { id: "ask_questions", desc: "Haz 6 o más preguntas en una partida", target: 6, reward: 100 },
  { id: "open_pack", desc: "Abre cualquier sobre en la Tienda", target: 1, reward: 60 },
  { id: "win_duel", desc: "Gana 1 Duelo TCG Online contra un vecino", target: 1, reward: 250 },
  { id: "play_duel", desc: "Disputa 1 Duelo TCG Online", target: 1, reward: 150 },
  { id: "upgrade_card", desc: "Mejora 1 cromo en el Taller Vecinal", target: 1, reward: 180 }
];
...
async function checkAndGenerateMissions(uid) {
  ...
  if (!snap.exists()) {
    // Mezclar y elegir 4 misiones aleatorias para el día para dar más variedad
    ...
    let dailyMissions = {
      ...
      mission3: { ...shuffled[2], progress: 0, claimed: false },
      mission4: { ...shuffled[3], progress: 0, claimed: false }
    };
...
function openMissionsUI(fromAlbum = null) {
  ...
  let mHtml = Object.keys(userAlbumData.missions).map(key => {
    ...
    return `
      <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:12px; display:flex; justify-content:space-between; align-items:center; width:100%; box-sizing:border-box;">
        <div style="text-align:left;">
          <div style="font-weight:bold; color:#fff; font-size:0.95rem; font-family:'Barlow', sans-serif;">${m.desc}</div>
          <div style="font-size:0.75rem; color:var(--text2); font-weight:bold; margin-top:2px;">Progreso: <span style="color:${isComplete ? '#4ade80' : '#facc15'};">${m.progress}/${m.target}</span></div>
        </div>
        <div>
          ${m.claimed
        ? `<span style="color:#4ade80; font-size:0.85rem; font-weight:bold;">✅ Reclamada</span>`
        : `<button class="auth-btn" ${!isComplete ? 'disabled style="opacity:0.4; cursor:not-allowed;"' : ''} onclick="claimMissionReward('${key}')" style="margin:0; background:${isComplete ? 'linear-gradient(135deg, #10b981, #059669)' : '#2563eb'}; color:#fff; font-weight:bold; font-size:0.78rem; padding:8px 12px; border-radius:8px; cursor:pointer;">🎁 Reclamar +${m.reward} 🪙</button>`
      }
        </div>
      </div>
    `;
  ...
  overlay.innerHTML = `
    <div class="auth-box" style="width:min(500px, 94vw); position: relative; border: 2px solid #8b5cf6; background:radial-gradient(circle at 50% 50%, #18113c 0%, #080516 100%); box-shadow:0 8px 30px rgba(0,0,0,0.8); box-sizing:border-box;">
      ...
      <div style="display:flex; flex-direction:column; gap:12px; width:100%;">
        ${mHtml || '<p style="color:var(--text2); text-align:center;">Cargando misiones...</p>'}
      </div>
    </div>
  `;
...
async function upgradeSignature(cardId) {
  ...
  if (typeof progressMission === 'function') {
    progressMission(uid, "upgrade_card", 1);
  }
...
async function surrenderCardDuel(roomCode) {
  ...
  await roomRef.transaction(room => {
    ...
    if (typeof progressMission === 'function') {
      ...
      progressMission(winnerUid, "play_duel", 1);
      progressMission(loserUid, "play_duel", 1);
    }
